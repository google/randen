...
#define NB_NOINLINE inline __attribute__((noinline))
...
template <typename T>
size_t MinRange(const T* const NB_RESTRICT sorted, const size_t idx_begin,
...
template <typename T>
T ModeOfSorted(const T* const NB_RESTRICT sorted, const size_t num_values) {
  size_t idx_begin = 0;
...
NB_NOINLINE FuncOutput EmptyFunc(const void* arg, const FuncInput input) {
  return input;
}
